  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/lead', {
        ...
        body: JSON.stringify({ ...form, source: 'contact', notes: form.message, website: honeypot }),
      })
      if (!res.ok) throw new Error()
      setStatus('success')
      GA.leadFormSubmit('contact_page')
    } catch {
      setStatus('error')
    }
  }
          ...
          <div className="flex items-start gap-4 bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
            ...
            <div>
              ...
              <a
                ...
                onClick={() => GA.phoneClick('contact_page')}
                className="text-brand-orange font-black text-xl hover:underline"
              >
                {COMPANY.phone}
              </a>
        ...
        {status === 'success' ? (
          <div className="text-center py-8">
            ...
            <p className="text-slate-600">
              ...
              <a
                href={COMPANY.phoneTel}
                onClick={() => GA.phoneClick('contact_success')}
